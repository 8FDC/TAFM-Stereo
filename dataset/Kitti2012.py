...
class Kitti2015(Dataset):
    def __init__(self, mode, size=(518, 518), scene:Literal['coral', 'default', 'industry', 'ship']='coral'):
        ...
        # if self.mode == 'train':
        #     self.left_image_paths = sorted(glob(rf'F:\dataset\kitti2015\testing\image_2\*.png'), key = lambda x: os.path.basename(x))
        #     self.right_image_paths = sorted(glob(rf'F:\dataset\kitti2015\testing\image_3\*.png'), key = lambda x: os.path.basename(x))
        #     self.left_normal_paths = sorted(glob(rf'F:\dataset\kitti2015\testing\normal_2\*.png'), key = lambda x: os.path.basename(x))
        #     self.disp_paths = ['' for _ in self.left_image_paths]
        # else:

        self.left_image_paths = sorted(glob(rf'F:\dataset\kitti2015\training\image_2\*.png'), key = lambda x: os.path.basename(x))
        self.right_image_paths = sorted(glob(rf'F:\dataset\kitti2015\training\image_3\*.png'), key = lambda x: os.path.basename(x))
        self.left_normal_paths = sorted(glob(rf'F:\dataset\kitti2015\training\normal_2\*.png'), key = lambda x: os.path.basename(x))
        self.disp_paths = sorted(glob(rf'F:\dataset\kitti2015\training\disp_noc_*\*.png'), key = lambda x: os.path.basename(x))

        assert len(self.left_image_paths) == len(self.right_image_paths) == len(self.disp_paths) == len(self.left_normal_paths)
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'val':
            disp = cv2.imread(disp_path, cv2.IMREAD_ANYDEPTH | cv2.IMREAD_ANYCOLOR) / 256
            disp[disp <= 0] = np.nan
