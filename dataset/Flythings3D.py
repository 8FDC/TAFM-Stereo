...
class Flythings3D(Dataset):
    def __init__(self, mode, size=(518, 518)):
        ...
        self.left_image_paths = sorted(glob(rf'F:\dataset\sceneflow\flyingthings3d\frames_finalpass\{"TRAIN" if self.mode == "train" else "TEST"}\*\*\left\*.png'))
        self.left_image_paths = [i for i in self.left_image_paths if 'normal' not in i]
        self.right_image_paths = sorted(glob(rf'F:\dataset\sceneflow\flyingthings3d\frames_finalpass\{"TRAIN" if self.mode == "train" else "TEST"}\*\*\right\*.png'))
        self.right_image_paths = [i for i in self.right_image_paths if 'normal' not in i]
        self.disp_paths = sorted(glob(rf'F:\dataset\sceneflow\flyingthings3d\disparity\{"TRAIN" if self.mode == "train" else "TEST"}\*\*\left\*.pfm'))
        self.left_normal_paths = [i.replace('.png', '_normal.png') for i in self.left_image_paths]
