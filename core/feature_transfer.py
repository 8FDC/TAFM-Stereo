...
class Feat_transfer(nn.Module):
    ...
    def forward(self, features):
        ...
        if feat_32x_up.shape[2:] != features[2].shape[2:]:
            feat_32x_up = F.interpolate(
                feat_32x_up, size=features[2].shape[2:], mode='bilinear', align_corners=False
            )

        feat_16x = self.conv16x(torch.cat((features[2], feat_32x_up), 1)) + self.res_16x(features[2])
        feat_16x_up = self.conv_up_16x(feat_16x)
        if feat_16x_up.shape[2:] != features[1].shape[2:]:
            feat_16x_up = F.interpolate(
                feat_16x_up, size=features[1].shape[2:], mode='bilinear', align_corners=False
            )

        feat_8x = self.conv8x(torch.cat((features[1], feat_16x_up), 1)) + self.res_8x(features[1])
        feat_8x_up = self.conv_up_8x(feat_8x)
        if feat_8x_up.shape[2:] != features[0].shape[2:]:
            feat_8x_up = F.interpolate(
                feat_8x_up, size=features[0].shape[2:], mode='bilinear', align_corners=False
            )
