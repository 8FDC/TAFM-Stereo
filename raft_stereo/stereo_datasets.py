...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='F:/dataset/stereo_matching_dataset', image_set='training'):

        # image_set = 'train' if image_set == 'training' else 'val'
        
        super(KITTI, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        assert os.path.exists(root)

        image1_list = sorted(glob(os.path.join(root, 'val', '*/color_left_*.jpg')))  # F:\dataset\stereo_matching_dataset\train\20260103_110535_797\color_left_20260103_110535_797.jpg
        image2_list = sorted(glob(os.path.join(root, 'val', '*/color_right_*.jpg')))
        disp_list = sorted(glob(os.path.join(root, 'val', '*/depth_*.png')))

        import random
        random.seed(0)
        selected_indices = random.sample(range(len(image1_list)), k=int(len(image1_list)*0.8))
        if image_set == 'training':
            image1_list = [image1_list[i] for i in selected_indices]
            image2_list = [image2_list[i] for i in selected_indices]
            disp_list = [disp_list[i] for i in selected_indices]
        else:
            image1_list = [image1_list[i] for i in range(len(image1_list)) if i not in selected_indices]
            image2_list = [image2_list[i] for i in range(len(image2_list)) if i not in selected_indices]
            disp_list = [disp_list[i] for i in range(len(disp_list)) if i not in selected_indices]
