...
class CorrBlock1D:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4, period_T=None):
        self.num_levels = num_levels
        self.radius = radius
        self.corr_pyramid = []
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            dx_base = torch.linspace(-r, r, 2*r+1).to(coords.device)
            
            if self.period_T is not None:
                T_level = self.period_T / (2 ** i)  # [B]
                T_level = T_level.view(batch, 1, 1, 1).to(coords.device)
                
                dx_base = dx_base.view(1, 1, 1, 2*r+1)
                
                dx_mid = dx_base + torch.zeros_like(T_level)  

                dx = torch.cat([dx_base - T_level, dx_mid, dx_base + T_level], dim=-1)
                
                coord_center = coords / 2**i
                
                # coord_center + dx: 
                # [B, h1, w1, 1] + [B, 1, 1, P] -> [B, h1, w1, P], 其中 P = 3 * (2*r+1)
                x0 = coord_center + dx
                
                x0 = x0.view(batch * h1 * w1, 1, dx.shape[-1])
            else:
                dx = dx_base.view(-1, 1) # [2*r+1, 1]
                x0 = dx + coords.reshape(batch*h1*w1, 1, 1, 1) / 2**i
                x0 = x0.view(batch * h1 * w1, 1, 2*r+1)
                
            y0 = torch.zeros_like(x0)

            coords_lvl = torch.stack([x0, y0], dim=-1).float()
