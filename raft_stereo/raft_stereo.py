...
class RAFTStereo_(nn.Module):
    ...
    def forward_feats(self, fmap1, fmap2, cnet_list, iters=12, flow_init=None, test_mode=False):
        """
        使用预先提取好的特征图做立体匹配。
        Args:
            fmap1, fmap2: 形状 [B, C, H, W] 的特征图（例如 match_left/right）
            cnet_list: 长度 n_gru_layers 的列表，每个元素是 [hidden_feat, context_feat]，
                        可直接传入 Feat_transfer_cnet 的输出。
        """

        with autocast(enabled=self.args.mixed_precision):
            net_list = [torch.tanh(x[0]) for x in cnet_list]  # 隐藏状态应该替换为 Conv(深度图)
            inp_list = [torch.relu(x[1]) for x in cnet_list]
