...
def warp_backward(match_images, base_disparities, invert=True):
    ...
    coordinates_x = base_disparities.new_tensor(range(base_disparities.shape[-1]))
    coordinates_y, coordinates_x = torch.meshgrid(coordinates_y, coordinates_x, indexing="ij")
    ...
    # Ensure grid has same dtype/device as input to avoid cudnn_grid_sampler_forward dtype mismatch
    coordinates = coordinates.to(dtype=match_images.dtype, device=match_images.device)
    # match_images = nn.functional.grid_sample(match_images, coordinates, mode="bilinear", padding_mode="border")
    match_images = nn.functional.grid_sample(
        match_images, coordinates, mode="bilinear", padding_mode="zeros", align_corners=True
    )

    ones = torch.ones_like(match_images[:, :1, ...])  # shape: [B,1,H,W]
    mask = nn.functional.grid_sample(
        ones, coordinates, mode="bilinear", padding_mode="zeros", align_corners=True
    )
    mask = (mask > 0.5)  # 比 0.999 更稳健
